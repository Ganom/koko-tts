...
export function useVirtualGrid(config: Ref<VirtualGridConfig>) {
  ...
  // Buffer around viewport to ensure smooth scrolling
  const bufferSize = computed(() => Math.max(config.value.iconSize * 2, 100));
  
  // Calculate logical grid bounds that need to be rendered
  ...
  // Generate icon data for a logical grid position
  const getIconAt = (logicalX: number, logicalY: number): VirtualIcon => {
    ...
    // Deterministic randomization based on logical position
  ...
  // Calculate currently visible icons
  ...
  watch(() => config.value.enableSlideAnimation, (enabled) => {
    if (enabled) {
      startAnimation();
    } else {
      stopAnimation();
  ...
  watch(() => [config.value.slideDirection, config.value.slideSpeed], () => {
    if (config.value.enableSlideAnimation) {
      stopAnimation();
      startAnimation();
    }
  ...
  // Calculate current rotation for animated icons
